...
function generateAddTaskOverlay(contacts) {
  let addTaskOverlayHTML = `
          ...
          <form class="taskSelector" id="taskSelector" onsubmit="createTask(true); return false;">
...
function taskHTML(taskData) {
  ...
  if (taskData.assignedContacts) {
    assignedContactsHTML = taskData.assignedContacts
      .map(
        (contact) => `
      <div class="taskContact" style="background-color: ${contact.color}">${contact.initials}</div>
    `
      )
      .join("");
  }
  return `
    <div class="taskCard" draggable="true">
      <div class="taskCategory">${taskData.category}</div>
      ...
      <div class="taskProgressContainer">
        ...
        <div class="taskProgressCount">${taskData.subtasks.length}</div>
      </div>
